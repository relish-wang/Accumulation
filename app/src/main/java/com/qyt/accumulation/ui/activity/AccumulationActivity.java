...
public class AccumulationActivity extends BaseActivity implements View.OnClickListener {
     ...
     * @param record 记录
     */
    private void onCancelClick(Record record) {
     ...
     * @param record 记录
     */
    private void onSaveClick(Record record) {
        View view = LayoutInflater.from(this).inflate(R.layout.dialog_add_record, null);
        EditText et_name = (EditText) view.findViewById(R.id.et_record_name);
        ...
        new AlertDialog.Builder(this)
                .setTitle("保存")
                .setView(view)
                .setPositiveButton("存入目标", (dialog, which) -> {
